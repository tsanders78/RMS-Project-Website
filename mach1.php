...
class FormGenerator extends Department {
    ...
    function advertise() {
        ...
        // Query to get top images from storeproduct based on popularity in productidpopularity
        $sqladvertise = "SELECT sp.longbob 
                         FROM storeproduct sp
                         JOIN productidpopularity pp ON sp.ProductID = pp.ProductID
                         ORDER BY pp.popularity DESC
                         LIMIT 6"; // Adjust the limit if you need more images
    ...
    public function renderHardwareAndRemodelingForm() {
        ...
        echo '        <h1>';
        // You can insert PHP code here if needed
        echo '        </h1>';
        echo '        <div align="center">';
        // You can insert PHP code here if needed
        echo '        </div>';
    ...
    function generateOrderSortForm() {
        return '
        <!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    function generateOrderListForm() {
        $html = '<!DOCTYPE html>
        <html lang="en">
        <head>
        <meta charset="UTF-8">
    ...
    function genereateSpreadSummary(){
    ...
    function handlePageNumber() {
        // Initialize page number from GET parameter
    ...
    function generatePaginationLinks() {
        ...
        $this->currentPage = $this->handlePageNumber();
    ...
    protected function generateRMSHardwareSiteFooterlinks() {
        ...
        foreach ($this->tables as $section => $tables) {
            foreach ($tables as $locate => $details) {
                $selected = ($locate === $this->compus) ? ' class="selected"' : '';
                echo ' <div class="fe-box" style="margin: 10px;">';
                echo ' <img src="' . htmlspecialchars($this->images[$locate]) . '" alt="' . htmlspecialchars($locate) . '">';
                echo ' <h6><a href="' . htmlspecialchars($_SERVER['PHP_SELF']) . '?location=' . htmlspecialchars($locate) . '"' . $selected . '>' . htmlspecialchars($locate) . '</a></h6>';
                echo ' </div>';
            }
        }
    ...
    function generateRMSHardwareSiteFooterFormat() {
    ...
    public function generateRMSHardwareSiteFooter() {
        ...
        // Assume $totalItems is the total number of items in your data source
        $totalItems = count($this->database_data[$this->table_name]);
        $totalPages = ceil($totalItems / $this->itemList);
        $currentPage = $this->page_number;
        $pageRange = 5; // Define the range of pages before and after the current page
        ...
        // Display numbers per page within the defined range
        for ($j = $startPage; $j <= $endPage; $j++) {
            echo '<div class="fe-box-duplicate">';
            echo '<h6>' . "||" . '</h6>'; // Display the number
    ...
    private function processTable($tableName, $operation, $data) {
        switch ($operation) {
            case 'insert':
    ...
    private function updateRecord($tableName, $data, $idColumn, $idValue) {
        $columns = array_intersect($this->tables[$tableName]['columns'], array_keys($data));
        $updates = [];
    ...
    private function deleteRecord($tableName, $idColumn, $idValue) {
...
protected $currentPage;
    ...
    protected $Data;
    protected $conn;
    protected $description; //Define description property
    protected $result; //Add this line to define the result property
    protected $row; //Define row property
    protected $length;
   protected $images = [
        'Home' => 'images/f1.png',
   ...
   protected $tables = [
    ...
    protected function fetchTableData($table_name, $columns) {
    ...
    public function loadData() {
        ...
        // else {
        //     echo "No tables found for the current location: " . $this->compus . "<br>";
        // }
    }
    
    public function displayImagesFromTable() {
        ...
        // else {
        //     echo "No data found for table: $this->table_name";
        // }
    }

    public function displayImage($imageData) {
        if (!empty($imageData)) {
...
class SpreadInput extends ItemSortHandler {
    public function __construct($usersearch = "", $itemstring = "", $itemVendor = "", $transactionNumber = 0, $numberOfItems = 0, $stockStatus = "", $discountCode = "", $discountPercentage = 0.0, $itemlist = "", $find = "", $itemsort = "") {
        ...
        // Assume these properties are initialized elsewhere in the class
    
        public function displayTablesForCurrentCompus() {
            if (isset($this->tables[$this->compus])) {
// Assuming $this->tables[$this->compus] is an associative array
...
for ($i = 0; $i < $this->length; $i++) {
    ...
    if ($table_name !== 'greet') {
        ...
        // Ensuring we do not exceed the number of items available
